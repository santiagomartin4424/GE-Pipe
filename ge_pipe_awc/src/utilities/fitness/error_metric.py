...
def balanced_accuracy(y, yhat):
    """
    Calculate balanced accuracy score between true labels and predictions.
    
    :param y: The expected input (true labels).
    :param yhat: The predicted labels from the model.
    :return: The balanced accuracy score (0.0 to 1.0).
    """
    
    # 1. Asegurar que y sea un array de numpy
    y = np.array(y)

    # 2. Manejar casos donde yhat es un escalar (individuo constante)
    if not isinstance(yhat, (np.ndarray, list)) or np.isscalar(yhat):
        yhat = np.full(y.shape, yhat)
    
    yhat = np.array(yhat)

    # 3. Solo aplicar umbral si estamos seguros de que son probabilidades binarias
    # Si son floats pero hay valores fuera de [0, 1] o es multiclase, 
    # es mejor confiar en lo que devuelve el modelo.
    if yhat.dtype.kind in 'fc': 
        # Si el dataset es binario (0, 1) y los valores son floats
        if len(np.unique(y)) <= 2:
            yhat = (yhat > 0.5).astype(y.dtype)
        else:
            # Para multiclase, redondeamos al entero más cercano
            yhat = np.round(yhat).astype(y.dtype)

    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        return balanced_accuracy_score(y, yhat)

balanced_accuracy.maximise = True
